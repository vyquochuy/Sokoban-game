...
class Game:
    ...
    def __init__(self, filename):
        ...
        with open(filename, 'r') as file:
            ...
            if line:
                ...
                try:
                    ...
                    print(
                        f"ERROR: The first line contains invalid values. Error details: {e}"
                    )
                    sys.exit(1)
            else:
                ...
                if line.strip() != "":
                    ...
                    for j, c in enumerate(line):
                        if c != '\n' and self.is_valid_value(c):
                            row.append(c)
                            if c in ['$', '*']:
                                self.stones_positions.append({
                                    'pos': (i, j),
                                    'weight':
                                    self.stones_weight[stone_id]
                                })
                                stone_id += 1
                        elif c == '\n':
                            continue
        ...
        self.wall = pygame.transform.scale(pygame.image.load("img/wall.png"),
                                           (self.tile_size, self.tile_size))
        self.Ares = pygame.transform.scale(pygame.image.load('img/Ares.png'),
                                           (self.tile_size, self.tile_size))
        self.floor = pygame.transform.scale(pygame.image.load('img/floor.png'),
                                            (self.tile_size, self.tile_size))
        self.stone = pygame.transform.scale(pygame.image.load('img/stone.png'),
                                            (self.tile_size, self.tile_size))
        self.stock = pygame.transform.scale(pygame.image.load('img/stock.png'),
                                            (self.tile_size, self.tile_size))
        self.Ares_on_stock = pygame.transform.scale(
            pygame.image.load('img/Ares on stock.png'),
            (self.tile_size, self.tile_size))
        self.stone_on_stock = pygame.transform.scale(
            pygame.image.load('img/stone on stock.png'),
            (self.tile_size, self.tile_size))
    ...
    def in_wall(self, x, y):
        ...
        if x == 0 or y == 0 or x == len(self.matrix) - 1 or y == len(
                self.matrix[x]) - 1:
            return False
        ...
        try:
            checks = [
                any(self.matrix[x][i] == '#'
                    for i in range(y + 1, len(self.matrix[x]))),  # phải
                any(self.matrix[i][y] == '#'
                    for i in range(x + 1, len(self.matrix))),  # xuống
                any(self.matrix[x][i] == '#'
                    for i in range(y - 1, -1, -1)),  # trái
                any(self.matrix[i][y] == '#'
                    for i in range(x - 1, -1, -1))  # lên
            ]
        except:
            return False
        return all(checks)
    ...
    def draw_map(self):
        ...
        for i in range(len(self.matrix)):
            for j in range(len(self.matrix[i])):
                ...
                if self.matrix[i][j] == '#':
                    ...
                    self.screen.blit(self.Ares,
                                     (j * self.tile_size, i * self.tile_size))
                elif self.matrix[i][j] == '.':
                    self.screen.blit(self.stock,
                                     (j * self.tile_size, i * self.tile_size))
                elif self.matrix[i][j] == '+':
                    self.screen.blit(self.Ares_on_stock,
                                     (j * self.tile_size, i * self.tile_size))
        ...
        pygame.draw.rect(
            self.screen, (155, 155, 111),
            (cost_box_x, cost_box_y, cost_box_width, cost_box_height))
        pygame.draw.rect(
            self.screen, (255, 255, 255),
            (cost_box_x, cost_box_y, cost_box_width, cost_box_height), 2)

        cost_text = f"Total Cost: {self.total_cost}"
        text_surface = self.font.render(cost_text, True, (255, 255, 255))
        text_rect = text_surface.get_rect(
            center=(cost_box_x + cost_box_width // 2,
                    cost_box_y + cost_box_height // 2))
        self.screen.blit(text_surface, text_rect)

        total_weight_text = f"Total Weight: {self.total_weight}"
        total_weight_surface = self.font.render(total_weight_text, True,
                                                (255, 255, 255))
        total_weight_rect = total_weight_surface.get_rect(
            center=(cost_box_x + cost_box_width // 2,
                    cost_box_y + cost_box_height // 2 + 20))
    ...
    def move(self, dx, dy):
        ...
        if self.matrix[new_x][new_y] in ['$', '*']:
            if self.can_push(new_x, new_y, dx, dy):
                # Di chuyển cục đá
                self.matrix[new_x][
                    new_y] = ' ' if self.matrix[new_x][new_y] == '$' else '.'
                self.matrix[new_x + dx][new_y + dy] = '*' if self.matrix[
                    new_x + dx][new_y + dy] == '.' else '$'
                for stone in self.stones_positions:
    ...
    def run_game(self, solution):
        ...
        if not solution:
            text_surface = font.render("UNSOLVABLE WITH CURRENT ALGORITHM",
                                       True, (255, 0, 0))
            text_rect = text_surface.get_rect(
        ...
        while running:
            ...
            for char in self.path:
                ...
                if self.is_win() and not win_message_displayed:
                    ...
                    # Button Close
                    button_color = (217, 109, 55)  # Màu nền của nút
                    border_color = (255, 255, 255)  # Màu viền
                    button_font = pygame.font.Font(None, 36)
                    button_text = button_font.render("Close", True,
                                                     (255, 255, 255))
